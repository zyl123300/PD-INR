...
def get_rmse(arr1, arr2, FOV_mask=None):
...
def get_mse_log(arr1, arr2, FOV_mask=None):
    # arr2 is the true image
    if FOV_mask is not None:
        arr1 = arr1[FOV_mask].copy()
        arr2 = arr2[FOV_mask].copy()
    ...
    # se = np.power(arr1 - arr2, 2)
    # mse = se.mean()
    return 10 * np.log10(np.sum((arr1 - arr2) ** 2) / np.sum(arr2 ** 2))
...
def pet_forward(H, im, n_subsets = 24):
    ...
    # print(H.proj_meta.shape)
    # sinogram = torch.zeros(H.proj_meta.shape).to(pytomography.dtype).to(H.output_device)
    #
...
class SystemMatrixGenerator:
    def __init__(self, path='data/mMR_Geometry.mac',atten_path=None, gpu_index=2, num_tof_bins=13, fwhm_tof_resolution_ps=200,
    ...
    def get_scanner_info(self):
        ...
        # info['moduleAxialNr'] = int(4)
        # info['crystalAxialNr'] = int(4)
        # info['NrRings'] = int(16)
        # info['radius'] = np.float32(200.0)
        return info
    ...
    def generate_system_matrix(self):
        ...
        if self.atten_path is not None:
            atten_map = bi_load(self.atten_path,self.object_shape).copy()
            atten_map = torch.tensor(atten_map).to(pytomography.device)/10
        else:
            atten_map=None
...
class MyLikelihood:
    ...
    def compute_atten_gradient(
        ...
        projections_predicted = self.system_matrix.forward(object, subset_idx) + additive_term_subset #\hat y_it
        projections_predicted2 = self.system_matrix.forward2(object, subset_idx)  #p_it
        return torch.sum(projections_predicted2 * ( (proj_subset / (projections_predicted+pytomography.delta)) - 1.0 ) , dim=-1)

    def _update_atten_coefficient(
            self,
            object: torch.Tensor,
            subset_idx: int | None = None,
    ) -> torch.Tensor:
